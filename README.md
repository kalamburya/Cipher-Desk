...
веб-сервис на Spring Boot и собственный интерфейс. 
> Это мой первый проект на Spring,
> да и вообще первый, поэтому он не идеален и не претендует на масштабность или серьёзность - 
> вместо базы данных используются HTTP сессии. Однако он является моим первым 
> шагом в мир программирования.

## Возможности

- **Два метода дешифровки/шифрования**
...
- Maven (с Maven Wrapper - `mvnw` / `mvnw.cmd` добавлены)

**Тестирование**
- JUnit 5
- Spring Boot Test / MockMvc - тестирование контроллера и HTTP-слоя
...
> Стоит честно отметить, что я не очень разбираюсь в css, поэтому за помощью в сложных 
...
- Обновление без перезагрузки сделано через `fetch` и `DOMParser`, а не REST и JSON: сервер
   рендерит готовый HTML, фронтенд просто подставляет нужные куски в текущую страницу.
...
```bash
git clone https://github.com/kalamburya/DecryptorWeb.git
cd DecryptorWeb
...
- Модульные тесты для алфавитов, дешифраторов, подстановщика и частотного анализа.
- Интеграционные тесты контроллера (`MockMvc`) - проверка HTTP-эндпоинтов, обработки ошибок и работы с историей сессии.
